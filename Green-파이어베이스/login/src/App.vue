<template>
  <v-app>
    ...
    <v-app-bar app clipped-left color="primary" dark>
      ...
      <v-toolbar-items class="hidden-xs-only"> <!-- class를 통해 xs(중단점)크기일때만 숨긴다   hidden-xs-only : xs 사이즈일때만 숨긴다-->
        <v-btn text v-for="(item,i) in fnGetMenuItems" :key="i" :to="item.to">
          <v-icon left>{{item.icon}}</v-icon> <!--left : 왼쪽으로 작게-->
          {{item.title}}
        </v-btn>
    ...
    <v-footer>
      <div class="mx-auto">&copy; CODE-DISIGN.web.app </div>  <!--app : 항상 위에있도록, class="mx-auto" 중앙정렬-->
    </v-footer>
